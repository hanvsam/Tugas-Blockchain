...
type Engine interface {
	...
	// Finalize menjalankan modifikasi status pasca-transaksi ( misalnya hadiah blok)
	// tetapi tidak merakit blok.
	//
	// Catatan: Header blok dan basis data status mungkin diperbarui untuk mencerminkan apa pun
	// aturan konsensus yang terjadi pada finalisasi ( misalnya hadiah blok).
	Finalize(chain ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
		uncles []*types.Header)

	// FinalizeAndAssemble menjalankan modifikasi status pasca-transaksi ( misalnya blok
